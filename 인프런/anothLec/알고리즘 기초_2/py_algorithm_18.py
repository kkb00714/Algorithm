...
# 그래서 현수는 격자의 행을 기준으로 
# 왼쪽, 또는 오른쪽으로 회전시켜 위치를 변경해 
# 모든 감이 잘 마르게 합니다.
# 만약 회전명령 정보가 2 0 3이면 
# 2번째 행을 왼쪽으로 3만큼 아래 그림처럼 
# 회전시키는 명령입니다.

# 첫 번째 수는 행번호, 두 번째 수는 방향인데 
# 0이면 왼쪽, 1이면 오른쪽이고, 
# 세 번째 수는 회전하는 격자의 수입니다.
# M개의 회전명령을 실행하고 난 후 
# 아래와 같이 마당의 모래시계 모양의 영역에는 
...
# 이 자연수는 각 격자안에 있는 감의 개수이며, 
# 각 격자안의 감의 개수는 100을 넘지 않는다.
...
for i in range(m):
    ...
    if t == 0: # (왼쪽으로 회전)
        for _ in range(k): # k만큼 회전 
            ...
            # 인덱스와 다르게 1번부터 시작하므로
            # pop과 동시에 list에 append함
            
    else: # (오른쪽으로 회전) => 오른쪽 요소들을 왼쪽으로 추가
        for _ in range(k):
